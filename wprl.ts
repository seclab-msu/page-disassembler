...
import { readTar } from './src/utils/read-tar';
import { DebundlerFS } from './src/debundler/debundler-fs';
import { hasattr, PARSE_OPTIONS } from './src/utils/common';
import { detectAndCutLibs } from './src/lib-detection.js';

function debundleSingleFileOnDiskAndExit(args: any) {
    const debundler = new DebundlerFS(args.target_dir);
    if (debundler.debundle(
        parse(fs.readFileSync(args.target, 'utf8'), PARSE_OPTIONS),
        args.target_dir)
    ) {
        debundler.runCallBackOnEachModule((name: string, module: string): string | null => {
...
async function main(args: any): Promise<void> {
    ...
    fs.writeFileSync(`${args.target_dir}/index.html`, page.resources['index.html']);
    ...
    for (let resourceName of Object.keys(page.resources)) {
        if (resourceName != 'index.html' && hasattr(page.resources, resourceName)) {
            let parsedNode;
            try {
                parsedNode = parse(page.resources[resourceName], PARSE_OPTIONS)
            } catch (e) {
                log(`Error occurred while parsing script ${resourceName}: ${e}`);
                continue;
            }
            if (debundler.debundle(parsedNode, resourceName)) {
                resourceName = resourceName.replace(/\//g, '_').replace(/\./g, '_')
                debundler.runCallBackOnEachModule((name: string, module: string): string | null => {
                    const fullPath = path.join(args.target_dir, resourceName, name + '.js');
                ...
                module = detectAndCutLibs(page.resources[resourceName], fullPath);
...
(async () => {
    ...
    argParser.add_argument('--heuristic-match',{ action: 'store_true' });
    argParser.add_argument('--exclude', '-x', { action: 'append', default: []});
